...
export default function HuntPage() {
    ...
    const { data: runs, mutate: refreshRuns } = useSWR('agent-runs', () => agent.runs(15));
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
                ...
                {result && (
                    <div className="space-y-4">
                        ...
                        <Card>
                            ...
                            <CardContent className="whitespace-pre-wrap text-sm">{result.answer}</CardContent>
                        </Card>
                                    ...
                                    {result.proposedActions.map((a, i) => (
                                        <div key={i} className="flex items-start justify-between gap-3 rounded border p-3">
                                            <div className="min-w-0">
                                                ...
                                                <pre className="mt-1 overflow-x-auto text-xs">{JSON.stringify(a.args, null, 2)}</pre>
                                            </div>
                                            <Button
                                                size="sm"
                            ...
                            <CardContent className="space-y-3">
                                ...
                                {result.steps.map((s, i) => (
                                    <div key={i} className="text-sm">
                                        ...
                                        {s.args != null && <pre className="mt-1 overflow-x-auto text-xs">{JSON.stringify(s.args)}</pre>}
                                        <div className="mt-1 max-h-40 overflow-x-auto rounded bg-muted/40 p-2 text-xs">{s.observation}</div>
            ...
            <div className="space-y-2">
                ...
                {runs?.map((r) => (
                    ...
                    >
                        <div className="line-clamp-2 text-xs">{r.question}</div>
