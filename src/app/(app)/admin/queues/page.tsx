...
import {
    Layers, Pause, Play, Trash2, RotateCcw, ChevronRight, MoreVertical,
} from 'lucide-react';
...
function rowTone(q: QueueStats): StatusKind | undefined {
    if (q.isPaused) return 'paused';
    if (q.failed > 0) return 'failed';
    if (q.active > 0) return 'active';
    return undefined;
}
...
function QueueRow({ queue: q, onChanged }: { queue: QueueStats; onChanged: () => void }) {
    ...
    return (
        <RowCard tone={rowTone(q)} muted={q.isPaused}>
            <CardContent className="py-3 flex items-center gap-4">
                ...
                >
                    ...
                    <div className="min-w-0">
                        <div className="flex items-center gap-2">
                            ...
                            {q.isPaused && <StatusBadge kind="paused">Paused</StatusBadge>}
                        </div>
                ...
                <div className="hidden sm:flex items-center gap-4 text-xs">
                    ...
                    <Stat label="active"  value={q.active}  tone="active" />
                    <Stat label="failed"  value={q.failed}  tone="failed" />
                    <Stat label="done"    value={q.completed} tone="idle" />
                </div>
